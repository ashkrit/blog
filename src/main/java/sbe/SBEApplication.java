...
public class SBEApplication {
    ...
    public static void main(final String[] args) throws Exception
    {
        ...
        /*
        MoreCarEncoder encoder = new MoreCarEncoder();
        encoder.wrapAndApplyHeader(directBuffer, 0, MESSAGE_HEADER_ENCODER);
        encoder.serialNumber(100);
        encoder.manufacturer("OK");

        int enc = encoder.encodedLength();

        System.out.println("Len " + enc);
        int index=MessageHeaderDecoder.ENCODED_LENGTH;

        System.out.println("Vaue " + directBuffer.getLong(index, LITTLE_ENDIAN) );
        index+=8;
        int length = directBuffer.getInt(index, LITTLE_ENDIAN);
        System.out.println("Vaue " + length);
        index+=4;

        byte[] data = new byte[length];
        directBuffer.getBytes(index,data,0,length);
        System.out.println("Vaue " +  new String(data));
        ...
        */
        ...
        final String encodingFilename = "data.bin";
        Path path = Paths.get(encodingFilename);
